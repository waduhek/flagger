...
func TestCreateJWT(t *testing.T) {
	...
	_, tokenErr := auth.CreateJWT("test_username")
	if tokenErr != nil {
...
func TestVerifyJWT(t *testing.T) {
	...
	t.Run("valid_jwt", func(subT *testing.T) {
		...
		_, verifyErr := auth.VerifyJWT(validJWT)
		if verifyErr != nil {
	...
	t.Run("incorrect_signing_method", func(subT *testing.T) {
		...
		_, verifyErr := auth.VerifyJWT(incorrectSignedJWT)
		if verifyErr == nil {
	...
	t.Run("expired_jwt", func(subT *testing.T) {
		...
		_, verifyErr := auth.VerifyJWT(expiredJWT)
		if verifyErr == nil {
